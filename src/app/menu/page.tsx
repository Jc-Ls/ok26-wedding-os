    ...
    <div style={{ width: '100vw', maxWidth: '100%', minHeight: '100vh', backgroundColor: 'var(--navy-bg)', paddingBottom: '128px', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'relative', width: '100%', paddingBottom: '30px', borderBottom: '1px solid rgba(212, 175, 55, 0.2)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '0 20px', marginTop: '10px' }}>
          <p style={{ fontSize: '0.85rem', color: '#fff', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '15px' }}>Welcome to the wedding ceremony of</p>
          <h1 style={{ fontFamily: '"Cormorant Garamond", serif', color: 'var(--gold-bright)', lineHeight: '1.2', marginBottom: '20px' }}>
            <span style={{ display: 'block', fontSize: '3rem' }}>Muhammed</span>
            <span style={{ fontFamily: '"Cinzel", serif', fontSize: '1.5rem', display: 'block', margin: '10px 0' }}>&</span>
            <span style={{ display: 'block', fontSize: '3rem' }}>Kaothar</span>
          </h1>
        </div>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: '600px', margin: '0 auto', padding: '0 16px', marginTop: '24px' }}>
        ...
        <div className="no-scrollbar" style={{ position: 'sticky', top: 0, zIndex: 20, backgroundColor: 'rgba(10, 20, 47, 0.95)', padding: '16px 0', marginBottom: '24px', overflowX: 'auto', whiteSpace: 'nowrap', borderBottom: '1px solid rgba(212, 175, 55, 0.2)' }}>
          <div style={{ display: 'flex', gap: '10px', padding: '0 4px' }}>
            {categories.map((cat) => (
              <button key={cat} onClick={() => setActiveTab(cat)} style={{ padding: '8px 18px', borderRadius: '9999px', fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', backgroundColor: activeTab === cat ? 'var(--gold-base)' : 'transparent', color: activeTab === cat ? '#000' : '#9ca3af', border: activeTab === cat ? 'none' : '1px solid #4b5563' }}>{cat}</button>
            ))}
          </div>
        </div>
              ...
              <div key={item.id}>
                <label className={`menu-item-card ${isSelected ? 'selected' : ''}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', borderRadius: '12px', border: `1px solid ${isSelected ? 'var(--gold-base)' : 'rgba(255,255,255,0.1)'}`, backgroundColor: isSelected ? 'rgba(212, 175, 55, 0.1)' : 'rgba(255,255,255,0.03)', cursor: 'pointer', width: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', flex: 1, paddingRight: '15px' }}>
                    ...
                    <span style={{ fontWeight: 'bold', color: '#fff', fontSize: '0.9rem', lineHeight: '1.4', textTransform: 'uppercase' }}>{item.name}</span>
                  </div>
                  <div style={{ width: '70px', height: '70px', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 10px rgba(0,0,0,0.5)' }}>
      ...
      {(selectedMeal || selectedDrink) && !orderStatus && !showIdentityPopup && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, width: '100%', padding: '16px', backgroundColor: 'rgba(10, 20, 47, 0.95)', borderTop: '1px solid rgba(212, 175, 55, 0.3)', zIndex: 30 }}>
          <button onClick={() => setShowIdentityPopup(true)} style={{ width: '100%', maxWidth: '600px', margin: '0 auto', display: 'block', backgroundColor: 'var(--gold-base)', color: '#000', fontWeight: 'bold', padding: '18px', borderRadius: '8px', textTransform: 'uppercase', letterSpacing: '2px', border: 'none', boxShadow: '0 0 20px rgba(212,175,55,0.3)', fontSize: '1rem' }}>
            Next Step
          </button>
        </div>
      )}
          ...
          <div className="tracker-card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', textAlign: 'left' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                <button onClick={() => setShowIdentityPopup(false)} style={{ flex: 1, padding: '12px', backgroundColor: 'transparent', color: '#fff', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '6px' }}>Cancel</button>
                <button onClick={handleConfirmOrder} disabled={isSubmitting} style={{ flex: 2, padding: '12px', backgroundColor: 'var(--gold-base)', color: '#000', fontWeight: 'bold', border: 'none', borderRadius: '6px', opacity: isSubmitting ? 0.7 : 1 }}>
