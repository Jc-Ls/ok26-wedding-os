  ...
  return (
    <div style={{ backgroundColor: '#050403', minHeight: '100vh', color: '#fff', overflowX: 'hidden', fontFamily: '"Montserrat", sans-serif' }}>
      <style>{`
        ...
        @keyframes pulseFade { 0%, 100% { opacity: 0.3; } 50% { opacity: 1; text-shadow: 0 0 15px rgba(229,193,88,0.5); } }
        @keyframes slowReveal { from { opacity: 0; filter: blur(5px); transform: scale(0.95); } to { opacity: 1; filter: blur(0); transform: scale(1); } }
        @keyframes slideUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes ticker { 0% { transform: translate3d(0, 0, 0); } 100% { transform: translate3d(-100%, 0, 0); } }
        @keyframes panImage { 0% { transform: scale(1.05) translateY(0); } 50% { transform: scale(1.1) translateY(-2%); } 100% { transform: scale(1.05) translateY(0); } }
        
        .lux-bg { background: linear-gradient(to bottom, rgba(26,22,18,0.8) 0%, #050403 100%); position: absolute; inset: 0; z-index: 1; }
        
        /* Placeholder Image Background Layer */
        .hero-img-layer { position: absolute; inset: 0; background-image: url('/images/placeholder-couple.jpg'); background-size: cover; background-position: center; opacity: 0.3; z-index: 0; animation: panImage 25s ease-in-out infinite; mix-blend-mode: luminosity; }
        ...
        .input-field { width: 100%; padding: 15px; margin-bottom: 15px; background: rgba(255,255,255,0.05); border: 1px solid rgba(229,193,88,0.3); border-radius: 8px; color: #fff; font-family: "Montserrat", sans-serif; outline: none; transition: 0.3s; }
      ...
      {/* --- GRAND HERO SECTION --- */}
      <section style={{ position: 'relative', width: '100%', minHeight: '90vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '20px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, width: '100%' }}>
          ...
          {/* 1. BISMILLAH (Highest Element) */}
          <div style={{ fontFamily: '"Amiri", serif', fontSize: '1.8rem', color: '#E5C158', marginBottom: '30px', textDecoration: 'underline', textUnderlineOffset: '8px', animation: 'fadeIn 2s ease-out' }}>
            بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
          </div>
          
          <div style={{ marginBottom: '30px', animation: 'fadeIn 2s ease-out 0.5s both' }}>
          ...
          {/* 2. PULSING OK"26 */}
          <h2 style={{ fontSize: '1.2rem', letterSpacing: '6px', color: '#E5C158', marginBottom: '20px', fontWeight: 'bold', animation: 'pulseFade 4s infinite ease-in-out' }}>
            OK"26
          </h2>
          
          {/* 3. REVEALING ARABIC TEXT */}
          <div style={{ fontFamily: '"Amiri", serif', fontSize: 'clamp(2.5rem, 10vw, 4rem)', color: '#E5C158', marginBottom: '-5px', textShadow: '0 4px 15px rgba(0,0,0,0.8)', animation: 'slowReveal 3s ease-out 1s both' }}>
            محمد الأول <span style={{ fontSize: '1.5rem', margin: '0 10px', color: '#fff' }}>&</span> الكوثر
          </div>
          
          {/* 4. SLIDE-UP ENGLISH NAMES */}
          <h1 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 'clamp(1.8rem, 8vw, 4.5rem)', color: '#fff', margin: 0, lineHeight: 1.2, whiteSpace: 'nowrap', animation: 'slideUp 1.5s ease-out 1.5s both' }}>
            MUHAMMEDUL'AWWAL <br/>
      ...
      <section style={{ padding: '60px 24px', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ position: 'relative', paddingLeft: '35px', borderLeft: '1px solid rgba(229,193,88,0.3)' }}>
          {/* Day 1 & 2 remain exactly matching your specs... */}
          <div className="itinerary-card">
            ...
            <Link href="/traditions" style={{ color: '#E5C158', fontSize: '0.85rem', textDecoration: 'none', borderBottom: '1px solid #E5C158', marginTop: '12px', display: 'inline-block', letterSpacing: '1px' }}>▶ WATCH: DISCOVER THE TRADITION</Link>
          </div>
      ...
      {/* --- SECURE RESERVATION DESK & DB TRIGGER --- */}
      <section style={{ padding: '60px 24px', backgroundColor: '#0A0806', borderTop: '1px solid rgba(229,193,88,0.2)' }}>
        <div style={{ maxWidth: '500px', margin: '0 auto' }}>
          ...
          <p style={{ color: '#aaa', fontSize: '0.85rem', textAlign: 'center', marginBottom: '30px', lineHeight: '1.6' }}>
          ...
          {formStatus === 'success' ? (
            <div style={{ padding: '30px 20px', backgroundColor: 'rgba(0, 250, 154, 0.05)', border: '1px solid #00FA9A', borderRadius: '12px', textAlign: 'center', animation: 'fadeIn 0.5s ease' }}>
              <div style={{ fontSize: '3rem', marginBottom: '10px' }}>✅</div>
              <h3 style={{ color: '#00FA9A', marginBottom: '15px', fontFamily: '"Cormorant Garamond", serif', fontSize: '1.8rem' }}>Reservation Confirmed</h3>
              <p style={{ color: '#fff', fontSize: '0.9rem', marginBottom: '20px' }}>Your details have been securely logged.</p>
              
              <div style={{ backgroundColor: '#000', padding: '15px', borderRadius: '8px', border: '1px dashed #E5C158', marginBottom: '20px' }}>
                <p style={{ color: '#aaa', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 5px 0' }}>Your Unique ID</p>
                <h2 style={{ color: '#E5C158', letterSpacing: '4px', margin: 0 }}>{reservationId}</h2>
              </div>
              
              <p style={{ color: '#aaa', fontSize: '0.8rem', fontStyle: 'italic' }}>*An automated email with this code is being dispatched to your inbox.</p>
            </div>
          ) : (
      ...
      <section style={{ padding: '50px 20px', textAlign: 'center', backgroundColor: '#050403' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '15px', maxWidth: '600px', margin: '0 auto', paddingBottom: '30px' }}>
          {RSVP_CONTACTS.map((contact, i) => (
            <a key={i} href={`tel:${contact.phone}`} style={{ color: '#fff', textDecoration: 'none', fontSize: '0.9rem', padding: '5px 10px', borderBottom: '1px solid rgba(229,193,88,0.3)' }}>{contact.phone.replace('+234', '0')}</a>
          ))}
        </div>
      </section>
      ...
      <footer style={{ padding: '30px 20px', textAlign: 'center', backgroundColor: '#050403', borderTop: '1px solid rgba(229,193,88,0.2)' }}>
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.7rem', letterSpacing: '2px', textTransform: 'uppercase', margin: 0 }}>
          Seamlessly Engineered by<br/><span style={{ color: '#E5C158', fontWeight: 'bold', fontSize: '0.9rem', marginTop: '5px', display: 'inline-block' }}>Jare's Choice Labs</span>
        </p>
      </footer>
    </div>
  );
