...
export default function HomePage() {
  ...
  const handleReservation = (e: React.FormEvent) => {
    ...
    // If you add a database later, the save logic goes here
    setFormStatus('success');
    setForm({ name: '', phone: '', email: '', location: '', passcode: '' });
  };

  if (!isMounted) return <div style={{ backgroundColor: '#050505', minHeight: '100vh' }} />;
  ...
  return (
    <div style={{ backgroundColor: '#050505', minHeight: '100vh', color: '#fff', overflowX: 'hidden', fontFamily: '"Montserrat", sans-serif' }}>
      <style>{`
        ...
        .lux-bg { background-image: radial-gradient(circle at center, #1a1a1a 0%, #050505 100%); position: absolute; inset: 0; z-index: 0; }
        
        .ticker-wrap { width: 100%; overflow: hidden; background: rgba(212,175,55,0.05); border-top: 1px solid rgba(212,175,55,0.2); border-bottom: 1px solid rgba(212,175,55,0.2); padding: 8px 0; position: relative; z-index: 20; }
        .ticker { display: inline-block; white-space: nowrap; padding-left: 100%; animation: ticker 30s linear infinite; color: #D4AF37; font-size: 0.85rem; letter-spacing: 2px; text-transform: uppercase; }
        ...
        .input-field { width: 100%; padding: 15px; margin-bottom: 15px; background: rgba(255,255,255,0.05); border: 1px solid rgba(212,175,55,0.3); border-radius: 8px; color: #fff; font-family: "Montserrat", sans-serif; outline: none; }
        .input-field:focus { border-color: #D4AF37; }
        .submit-btn { width: 100%; padding: 15px; background: #D4AF37; color: #050505; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; transition: 0.3s; }
        .submit-btn:hover { background: #b5952f; }
      `}</style>
      ...
      <section style={{ position: 'relative', width: '100%', minHeight: '85vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '20px' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, animation: 'fadeIn 2s ease-out', width: '100%' }}>
          ...
          <div style={{ marginBottom: '30px' }}>
            <p style={{ fontFamily: '"Montserrat", sans-serif', fontSize: '0.65rem', color: '#D4AF37', letterSpacing: '3px', textTransform: 'uppercase', margin: '0 0 5px 0' }}>THE FAMILIES:</p>
          ...
          <h2 style={{ fontSize: '1rem', letterSpacing: '4px', color: '#D4AF37', marginBottom: '20px', fontWeight: 'bold' }}>OK"26</h2>
          
          {/* Arabic Text */}
          <div style={{ fontFamily: '"Amiri", serif', fontSize: 'clamp(2rem, 8vw, 3.5rem)', color: '#D4AF37', marginBottom: '-5px', textShadow: '0 4px 10px rgba(0,0,0,0.5)' }}>
            محمد الأول <span style={{ fontSize: '1.5rem', margin: '0 10px', color: '#fff' }}>&</span> الكوثر
          </div>
          
          {/* Responsive English Names using clamp() */}
          <h1 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: 'clamp(1.8rem, 8vw, 4.5rem)', color: '#fff', margin: 0, lineHeight: 1.2, whiteSpace: 'nowrap' }}>
            MUHAMMEDUL'AWWAL <br/>
            <span style={{ fontSize: 'clamp(1.5rem, 5vw, 3rem)', color: '#D4AF37' }}>&</span> KAOTHAR
          </h1>
        </div>
      </section>
      ...
      <section style={{ padding: '40px 20px 0', position: 'relative', zIndex: 20 }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', backgroundColor: 'rgba(15,15,15,0.9)', border: '1px solid rgba(212,175,55,0.3)', borderRadius: '15px', padding: '20px', display: 'flex', justifyContent: 'space-around' }}>
          {[ 
      ...
      <section style={{ padding: '60px 24px', maxWidth: '600px', margin: '0 auto' }}>
        <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '2.5rem', color: '#D4AF37', textAlign: 'center', marginBottom: '40px' }}>Venue & Event</h2>
        
        <div style={{ position: 'relative', paddingLeft: '35px', borderLeft: '1px solid rgba(212,175,55,0.3)' }}>
          
          {/* DAY 1 */}
          <div className="itinerary-card">
            <div style={{ position: 'absolute', left: '-41px', top: '5px', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#D4AF37' }} />
            <h5 style={{ color: '#D4AF37', fontSize: '1rem', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '5px' }}>THUR 25 JUNE</h5>
            ...
            <Link href="/traditions" style={{ color: '#D4AF37', fontSize: '0.8rem', textDecoration: 'none', borderBottom: '1px solid #D4AF37', marginTop: '10px', display: 'inline-block' }}>▶ Watch: Discover the Tradition</Link>
          </div>

          {/* DAY 2 */}
          <div className="itinerary-card">
            <div style={{ position: 'absolute', left: '-41px', top: '5px', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#D4AF37' }} />
            <h5 style={{ color: '#D4AF37', fontSize: '1rem', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '5px' }}>FRIDAY 26 JUNE</h5>
          ...
          <div className="itinerary-card">
            <div style={{ position: 'absolute', left: '-41px', top: '5px', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#D4AF37' }} />
            <h5 style={{ color: '#D4AF37', fontSize: '1rem', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '5px' }}>SATURDAY 27 JUNE</h5>
      ...
      <section style={{ padding: '40px 24px', backgroundColor: '#0A0A0A', borderTop: '1px solid rgba(212,175,55,0.2)' }}>
        <div style={{ maxWidth: '500px', margin: '0 auto' }}>
          <h2 style={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '2.2rem', color: '#D4AF37', textAlign: 'center', marginBottom: '10px' }}>Secure Your Seat</h2>
          ...
          {formStatus === 'success' ? (
            ...
            <form onSubmit={handleReservation}>
              ...
              <div style={{ position: 'relative' }}>
                <input type="password" placeholder="Enter VIP Passcode (Required)" required value={form.passcode} onChange={(e) => setForm({ ...form, passcode: e.target.value })} className="input-field" style={{ borderColor: formStatus === 'error' ? '#ff4444' : 'rgba(212,175,55,0.3)' }} />
      ...
      {/* --- QUOTE & RSVP FOOTER --- */}
      <section style={{ padding: '50px 20px', textAlign: 'center', backgroundColor: '#050505' }}>
        ...
        <h4 style={{ color: '#D4AF37', letterSpacing: '2px', fontSize: '1.2rem', marginBottom: '20px' }}>R.S.V.P</h4>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '15px', maxWidth: '600px', margin: '0 auto' }}>
          {RSVP_CONTACTS.map((contact, i) => (
            <a key={i} href={`tel:${contact.phone}`} style={{ color: '#fff', textDecoration: 'none', fontSize: '0.9rem', padding: '5px 10px', borderBottom: '1px solid rgba(212,175,55,0.3)' }}>
